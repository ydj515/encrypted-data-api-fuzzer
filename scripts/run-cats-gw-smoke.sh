...
API="${API:-}"
DEFAULT_CONTRACT_PATH="$(bash "$SCRIPT_DIR/resolve-cats-contract.sh" "$ORG" "$SERVICE" smoke)"
...
환경변수(선택):
  CONTRACT_PATH  OpenAPI 파일 경로 (기본: ORG/SERVICE에 맞는 gateway 계약 자동 선택, smoke 전용 파일이 없으면 full 계약 사용)
