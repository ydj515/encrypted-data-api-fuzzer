@service=reservation @api=getResourceInventory @kind=single-api
Feature: orgA reservation 자원 재고 확인(본문 기반) 단건 API 테스트
  ...
  Scenario: OpenAPI maxLength 필드 resourceId 최대 길이 초과 시 400 반환
    Given path basePath + '/getResourceInventory'
    And request { resourceId: 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', date: '2026-02-26' }
    When method POST
    Then status 400
