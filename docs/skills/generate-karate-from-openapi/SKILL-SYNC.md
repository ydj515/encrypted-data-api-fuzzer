...
이 문서는 동일한 스킬을 Codex(`.codex/`)와 Claude Code(`.claude/`) 양쪽에서 유지할 때 싱크를 맞추는 절차를 정의합니다. 어느 쪽에서 수정하든 이 문서를 읽고 대응 항목을 함께 업데이트하세요.
...
.codex/skills/generate-karate-from-openapi/
  SKILL.md                          ← Codex 전용 frontmatter + 공통 본문
  agents/openai.yaml                ← Codex 전용 (Claude Code에 없음)
  references/
...
.claude/skills/generate-karate-from-openapi/
  SKILL.md                          ← Claude Code 전용 frontmatter + 공통 본문
  references/
    negative-case-generation.md     ← 양쪽 독립 파일. 한쪽 수정 시 다른 쪽도 반영
    project-karate-patterns.md
    scenario-planning.md
...
| 섹션 | `.codex/SKILL.md` | `.claude/SKILL.md` | 비고 |
|---|---|---|---|
| frontmatter `name` | `name:` | `name:` | 항상 동일하게 유지 |
| frontmatter `description` | Codex 문법 | Claude Code 문법 | 도구별 독립 편집 가능 |
| Overview | Overview | Overview | 내용 동일해야 함 |
| Quick Start | Quick Start | Quick Start | 내용 동일해야 함 |
...
| Validation (스크립트) | `quick_validate.py` 명령 | 없음 | Codex 전용 |
| Validation (테스트) | `./gradlew test` 예시 | `./gradlew test` 예시 | 내용 동일해야 함 |
| `agents/openai.yaml` | 있음 | 없음 | Codex 전용 |
...
- [ ] `.codex/skills/generate-karate-from-openapi/SKILL.md`의 동일 섹션을 같은 내용으로 업데이트한다.
- [ ] 두 파일 모두 `> **싱크 안내:**` 블록 인용을 그대로 유지한다.
- [ ] 수정한 섹션이 `핵심 규칙` 목록이면, 규칙 수와 순서가 두 파일에서 일치하는지 확인한다.

**Codex 사용자가 `.codex/skills/.../SKILL.md`를 수정한 경우:**
...
- [ ] `.codex/SKILL.md`와 `.claude/SKILL.md` 모두에서 `name:`을 같은 값으로 변경한다.
- [ ] `.codex/agents/openai.yaml`의 `display_name`이 사람이 읽기 좋은 형태로 유지되는지 확인한다.
...
- [ ] `.codex/skills/generate-karate-from-openapi/references/<파일명>.md`에 동일 변경을 적용한다.

**Codex 사용자가 `.codex/skills/.../references/<파일명>.md`를 수정한 경우:**
...
- [ ] `.codex/skills/generate-karate-from-openapi/scripts/generate_karate_features.py`에 동일 변경을 적용한다.

**Codex 사용자가 `.codex/skills/.../scripts/generate_karate_features.py`를 수정한 경우:**
...
이 파일은 Codex 전용입니다. Claude Code에 대응 파일이 없으므로 `.codex/` 쪽만 수정하면 됩니다.
...
# Overview 섹션 비교
diff \
  <(sed -n '/^## Overview/,/^## /p' .codex/skills/generate-karate-from-openapi/SKILL.md | head -n -1) \
  <(sed -n '/^## Overview/,/^## /p' .claude/skills/generate-karate-from-openapi/SKILL.md | head -n -1)

# Workflow 섹션 비교
diff \
  <(sed -n '/^## Workflow/,/^## /p' .codex/skills/generate-karate-from-openapi/SKILL.md | head -n -1) \
  <(sed -n '/^## Workflow/,/^## /p' .claude/skills/generate-karate-from-openapi/SKILL.md | head -n -1)

# Repository Conventions 섹션 비교
diff \
  <(sed -n '/^## Repository Conventions/,/^## /p' .codex/skills/generate-karate-from-openapi/SKILL.md | head -n -1) \
...
```bash
diff .codex/skills/generate-karate-from-openapi/references/project-karate-patterns.md \
     .claude/skills/generate-karate-from-openapi/references/project-karate-patterns.md

diff .codex/skills/generate-karate-from-openapi/references/scenario-planning.md \
     .claude/skills/generate-karate-from-openapi/references/scenario-planning.md

diff .codex/skills/generate-karate-from-openapi/references/negative-case-generation.md \
...
```bash
diff .codex/skills/generate-karate-from-openapi/scripts/generate_karate_features.py \
...
1. `.codex/skills/<skill-name>/`에 Codex 구조를 작성한다.
2. `.claude/skills/<skill-name>/SKILL.md`를 Claude Code 포맷으로 작성한다.
3. 공유 참조 파일과 스크립트가 있으면 양쪽 `references/`, `scripts/` 모두에 실제 파일로 복사한다.
4. 이 문서를 복사하여 `docs/skills/<skill-name>/SKILL-SYNC.md`를 만들고 각 섹션을 해당 스킬에 맞게 수정한다.
5. 각 SKILL.md에 `> **싱크 안내:**` 블록 인용을 추가하여 SKILL-SYNC.md를 가리키게 한다.
