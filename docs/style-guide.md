...
- **생성자**: 생성자 노출은 최소화한다. Spring bean은 생성자 주입을 기본으로 하고, 도메인/DTO 생성은 public 생성자보다 정적 팩토리 메서드나 builder 형태를 우선한다.
- **Null 처리**: 들어오는 값은 최대한 validation/방어적 체크. 외부 응답 파싱 시 기본값을 안전하게 처리.
- **테스트 용이성**: 비즈니스 로직, 파싱, 검증, 파일/외부 연동 경계는 분리해 작성한다. 시간, 파일시스템, 네트워크, 환경변수처럼 테스트에서 제어하기 어려운 값은 주입하거나 작은 어댑터로 감싸 단위 테스트가 가능하게 한다.

## Silent Failure 지양
- **조용한 대체 금지**: 파싱 실패, 잘못된 설정, 유효하지 않은 입력을 임의의 기본값(`now()`, 빈 문자열, 0 등)으로 조용히 대체하지 않는다.
- **명시적 처리**: 계속 진행 가능한 문제는 WARN 로그와 함께 스킵/부분 처리하고, 데이터 정합성을 해치는 문제는 예외를 던져 호출자가 인지하게 한다.
- **문맥 포함**: 예외/로그에는 원인 파악에 필요한 식별자(`runId`, `org`, `service`, `api`, 파일 경로 등)를 포함하되 민감정보는 제외한다.
- **테스트**: 실패해야 하는 입력은 단위 테스트로 고정해 회귀를 막는다.
...
- **단위 테스트 필수**: 파서, 저장소, 검증 로직, 예외 처리처럼 분기나 데이터 변환이 있는 코드는 단위 테스트를 작성한다. 버그 수정 시에는 재발 방지 테스트를 먼저 또는 함께 추가한다.
- **테스트 가능한 설계**: static 전역 상태, 직접적인 현재 시각 조회, 하드코딩된 경로/환경변수 접근, 실제 네트워크 호출을 핵심 로직 안에 숨기지 않는다. 필요한 경우 생성자 주입, 인터페이스, 작은 helper 클래스로 분리한다.
- **경계값 테스트**: 잘못된 입력, 누락된 필드, 빈 값, path traversal 가능 값, 외부 포맷 변경 등 실패/경계 케이스를 포함한다.
